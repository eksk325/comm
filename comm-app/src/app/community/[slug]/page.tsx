    ...
    return (
      <div className={`flex-center-col items-center ${notoJP.className}`}>
        <div className="container">
          <span className="text-sm text-center">{`< ${post.tag} >`}</span>
          <span className="text-2xl mb-2 font-medium">{post.title}</span>
          <span className="text-sm mb-3">
            {`${post.username} (${getTimeAgo(post.timestamp)})`}
          </span>

          <span className="text-sm py-3 w-full border-t-[1px] font-light border-zinc-800 whitespace-pre-line	">
            {post.content}
          </span>
